    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          <!-- imageChange 自定义事件 在imglist里触发 传index值 然后更改传给zoom的图片 父里computed -->
          <!-- 或者不通过父 直接绑全局事件总线 zoom mounted的时候绑定自定义事件 imagechange 
          然后imglist里触发imagechange事件 传只要传image的地址就行了吧 -->
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="attr in spuSaleAttrList" :key="attr.id">
                ...
                <dd 
                  ...
                  @click="changeActive(attrValue,attr.spuSaleAttrValueList,index)">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input v-model="skuNum" autocomplete="off" class="itxt">
...
export default {
    ...
    computed:{
        ...mapGetters('detail',['categoryView','spuSaleAttrList','skuInfo','skuImageList'])
    },
    methods:{
      changeActive(value,valueList){
        // 如果本身就已经选中
        // 注意此处的坑是isChecked是字符串1
